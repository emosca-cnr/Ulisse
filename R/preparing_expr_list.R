...
#' @description `preparing_expr_list()` produce the cluster list needed for CCC calculation
...
#' @param clusters a vector with cluster membership of all the cells in `mtx`
#' @param universe names of the genes of interest. Used to filter the cluster gene lists
#' @param mean_t threshold on the expression used to binarize the expression matrix. This value should be decided considering 
#' the distribution of the mean expression of all the genes (without considering the zeros)
#' @param cell_t minimum number of cells in which a gene should be expressed over `mean_t` threshold in a cluster
...
preparing_expr_list <- function(mtx, clusters, mean_t=1, 
                              cell_t = 5, universe) {
  mtx <- as.matrix(mtx)
